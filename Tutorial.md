...
1. ホーム画面に遷移 → 説明文を表示すると同時に挨拶イベントを発生
2. 少し時間を置いて、イベント内容の説明ポップアップを表示
...
1. ステップ3の2つ目のポップアップが表示されたタイミングで、ホーム画面に困りごと相談イベント（Choice形式）を1件発生させる

   * 信頼度変化は固定（信頼度上昇）
2. ステップ3最後のポップアップを閉じてから3秒後、以下の誘導文を表示する
3. プレイヤーが対応を完了した後、完了メッセージとあわせて追加の説明文を表示
